...
class LightService:
    ...
    def generate_exercise(self, task_img: Image, mode: str) -> int:
        ...
        task_img = self.blender_file_service.get_zero_image_light()
        draw_task = ImageDraw.Draw(task_img)
        empty_img = Image.new(mode="RGB", size=answer_img.size, color=(255, 255, 255))

        for i in range(len(coords)):
            draw_task.text((10, i * 100), f"{self.coord_map[coords[i]]}: {val[i]}", (0, 0, 0), self.font)
        ...
        self.data_service.add_exercise_info(ExerciseInformationDto(
            exercise_id, "root", "LightShadow", mode, int(datetime.datetime.now().timestamp())
        ))
    ...
    def score_exercise(self, exercise_id: int, subextype: str) -> None:
        ...
        score = self.scorer_service.get_abs_dist_score(sub_img, sol_img)
